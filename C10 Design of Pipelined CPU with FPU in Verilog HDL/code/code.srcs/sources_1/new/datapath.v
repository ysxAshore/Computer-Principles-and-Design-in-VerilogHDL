...
module datapath(
    ...
    //IF/ID
    output [5:0]op,funct,
    output [4:0]rs,rt,fs,ft,fd,
    input sext,regdst,fwdf,
    input [1:0]forwardAD,forwardBD,
    output z,

    //ID/EX
    input eshift,ealusrc,ejal,swfp,efwdfe,
    input [3:0]ealuc,
    input [31:0]e3d,dfb,
    output [4:0]ewn,
    ...
    input wm2reg,wwreg,
    output [4:0]wwn,
    output [31:0]wd,aluoutE,memreadW
);
    ...
    assign fs=instrD[15:11];
    assign ft=instrD[20:16];
    assign fd=instrD[10:6];
    ...
    wire [31:0]writememData_tempD,writememDataD;
    mux2to1 generate_WMemDtemp(B,dfb,swfp,writememData_tempD);
    mux2to1 generate_WMemD(writememData_tempD,e3d,fwdf,writememDataD);
    assign rpc=A;
    assign z=~|(A^B);//判断0时，刚开始写成了~(A^B)是对A^B所有位取反,需要先连续|得到的结果再取反~|

    //ID/EXE
    wire [31:0]pcAdder4E,eA,eB,eimmSext,writememDataE,writememData_tempE;
    wire [4:0]ewn_temp;
    regDesign #(.WIDTH(165))id_ex_init(clk,1'b1,clrn,{pcAdder4D,A,B,imm_sext,wn_temp,writememDataD},{pcAdder4E,eA,eB,eimmSext,ewn_temp,writememData_tempE});//位宽刚开始数错了，也要清0
    ...
    mux2to1 generate_memWriteData(writememData_tempE,e3d,efwdfe,writememDataE);
    assign ewn=ewn_temp|{5{ejal}};
    //EX/MEM
    regDesign #(.WIDTH(69)) ex_mem_init(clk,1'b1,clrn,{aluoutE,writememDataE,ewn},{aluoutM,memWriteData,mwn});//也要清0
    //MEM/WB
    wire [31:0]aluoutW;
